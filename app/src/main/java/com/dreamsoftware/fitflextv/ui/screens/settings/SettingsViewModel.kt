...
class SettingsViewModel @Inject constructor() : ViewModel() {
    ...
    fun updateSetting(item: SettingsItemUIState) {
        ...
        val personalSetting =
            SettingsMenuUIState.PersonalSettings.entries.find { it.name == item.title }

        appSetting?.value = item.value
        personalSetting?.value = item.value

        val updatedAppSettings =
            _settingsUIState.value.appSettings.second.map { if (it.title == item.title) item else it }
        val updatedPersonalSettings =
            _settingsUIState.value.personalSettings.second.map { if (it.title == item.title) item else it }
